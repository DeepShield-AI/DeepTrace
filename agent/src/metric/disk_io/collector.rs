...
impl DiskCollector {
	pub fn new() -> Self {
		Self {
			devices: Self::get_block_devices("/sys/block")
				.unwrap_or_else(|_| vec!["sda".to_string()]),
		}
	}
	...
	fn get_block_devices(block_path: &str) -> Result<Vec<String>, ()> {
		...
		if devices.is_empty() { Err(()) } else { Ok(devices) }
	}
			...
			if device.starts_with("/dev/sd") || device.starts_with("/dev/nvme") {
				...
				match statvfs(mount_point) {
					Ok(stat) => {
						...
						let use_percent =
							if size > 0 { (used as f64 / size as f64) * 100.0 } else { 0.0 };
	...
	pub fn collect_ext4_cache_stats(&self) -> Result<Ext4CacheStats, String> {
		...
		// 获取所有块设备
		let devices =
			Self::get_block_devices("/proc/fs/ext4").map_err(|_| "Failed to get block devices")?;
		for device in devices {
			...
			if let Ok(contents) = fs::read_to_string(&es_shrinker_info_path) {
				println!("entered es_shrinker_info_path");
		...
		Ok(Ext4CacheStats { hits: total_hits, misses: total_misses })
	}
	...
	pub fn calculate_io_times(metrics: &mut [DiskMetric]) {
		for metric in metrics.iter_mut() {
			...
			if metric.time_spent_io > 0 {
				metric.aqu_sz =
					(metric.weighted_time_spent_io as f64) / (metric.time_spent_io as f64);
			} else {
				metric.aqu_sz = 0.0;
			}
		}
