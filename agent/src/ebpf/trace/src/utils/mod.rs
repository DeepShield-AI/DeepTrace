...
unsafe fn tcp_sock_from_fd_inner(fd: u64) -> Result<usize, u32> {
	...
	if unsafe { bpf_probe_read(files as *mut _, size_of::<files_struct>() as u32, (*task).files as *const _) } !=
		0
	{
		return Err(0);
	}
	...
	if unsafe { bpf_probe_read(fd_table as *mut _, size_of::<fdtable>() as u32, (*files).fdt as *const _) } !=
		0
	{
		return Err(0);
	}
