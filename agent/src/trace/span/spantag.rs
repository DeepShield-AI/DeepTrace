...
use trace_common::structs::{Data, Direction};
...
impl DockerTag {
    ...
    pub async fn get_docker_tags(tgid: u32) -> Option<DockerTag> {
        ...
        for container in containers {
            ...
            let docker_tgid = inspect.state.as_ref()
                .and_then(|s| s.pid)
                .unwrap_or(0) as u32;
                // 获取 IPAddress
            let ip_address = inspect.network_settings.as_ref()
                .and_then(|ns| ns.networks.as_ref())
                .and_then(|networks| networks.values().next().and_then(|net| net.ip_address.clone()))
                .unwrap_or_default();
            ...
            let tag = DockerTag {
                ...
                hostname: inspect.config.as_ref().and_then(|c| c.hostname.clone()).unwrap_or_default(),
                gateway: inspect.network_settings.as_ref()
                    .and_then(|ns| ns.networks.as_ref())
                    .and_then(|networks| networks.values().next().and_then(|net| net.gateway.clone()))
                    .unwrap_or_default(),
                ip: ip_address,
