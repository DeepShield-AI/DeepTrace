...
func apiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if pathType == "1" {
        service4Resp, err := callService("http://service4:8080/api4", requestID, pathType, serviceIdx)
        if err != nil {
        ...
        // 调用 service-c
        service5Resp, err := callService("http://service5:8080/api5", requestID, pathType, serviceIdx)
        if err != nil {
        ...
        if err != nil {
            http.Error(w, "Failed to call service6: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s %s %s", "service1", service4Resp, service5Resp, service6Resp)
    }
    ...
    if pathType == "2" {
        ...
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s", "service1", service5Resp)
    }
