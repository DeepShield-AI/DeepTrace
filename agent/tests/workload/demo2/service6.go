...
func apiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if pathType == "3" {
        ...
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s %s", "service6", service12Resp, service13Resp)
    }
    ...
    if pathType == "5" {
        service11Resp, err := callService("http://service11:8080/api11", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service-c: "+err.Error(), http.StatusInternalServerError)
            return
        }
        ...
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s %s", "service6", service11Resp, service14Resp)
    }
...
func main() {
    http.HandleFunc("/api6", apiHandler)
