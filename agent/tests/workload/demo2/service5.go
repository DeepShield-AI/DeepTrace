...
func apiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if pathType == "1" {
        service12Resp, err := callService("http://service12:8080/api12", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service4: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s", "service5", service12Resp)
    }
    ...
    if pathType == "3" {
        service10Resp, err := callService("http://service10:8080/api10", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service-c: "+err.Error(), http.StatusInternalServerError)
            return
        }
        service13Resp, err := callService("http://service13:8080/api13", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service-c: "+err.Error(), http.StatusInternalServerError)
            return
        }

        service15Resp, err := callService("http://service15:8080/api15", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service-c: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s %s %s", "service5", service10Resp, service13Resp, service15Resp)
    }
...
func main() {
    http.HandleFunc("/api5", apiHandler)
