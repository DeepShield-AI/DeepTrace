...
impl Stats {
	async fn new() -> Self {
		...
		Self { name, lcuuid, cpu_usage: cpu_usage.into(), memory_usage, timestamp, span_num }
	}
...
pub(super) async fn health_checker() {
	...
	loop {
		...
		// 先检查是否存在该文档
		let exists = client
			.get(elasticsearch::GetParts::IndexId(es_index_name, state_id.as_str()))
			.send()
			.await
		...
		let response = if exists {
			...
			client
				.update(elasticsearch::UpdateParts::IndexId(es_index_name, state_id.as_str()))
				.body(json!({
					"doc": state,
					"doc_as_upsert": true
				}))
			...
			client
				.index(elasticsearch::IndexParts::IndexId(es_index_name, state_id.as_str()))
				.body(json!(state))
				.send()
				.await
