...
def install_db():
    ...
    elastic_pwd, kibana_pwd = read_db_config()
    ...
    # 启动服务
    run(f"sudo docker-compose -f {compose_file} up -d")
...
def uninstall_db():
    ...
    subprocess.run(f"sudo docker-compose -f {compose_file} down", shell=True, check=True)
