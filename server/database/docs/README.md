# Deployment

1. Set the `kibana_password` and `elastic_password` in the [configuration file](../../config/config.toml)
2. Run: `python3 -m database.src.deploy`
3. Access: `http://ip:5601`  
   Username: `elastic`  
   Password: `elastic_password`

# Uninstallation
Run `python3 -m database.src.clear`

# Testing
- This test stores spans generated by a microservice with three components under different RPS (requests per second) into different database tables. You can view the results through the Kibana frontend.

```bash
python3 -m database.test.database
```