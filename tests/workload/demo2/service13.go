...
func apiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if pathType == "1" {
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s", "service13")
    }

    if pathType == "3" {
        service20Resp, err := callService("http://service20:8080/api20", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service4: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s", "service13", service20Resp)
    }


    if pathType == "4" {
        service19Resp, err := callService("http://service19:8080/api19", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service4: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s", "service13", service19Resp)
    }
}

func main() {
    http.HandleFunc("/api13", apiHandler)
