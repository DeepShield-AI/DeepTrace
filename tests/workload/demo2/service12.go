...
func apiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if pathType == "1" {
        service18Resp, err := callService("http://service18:8080/api18", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service4: "+err.Error(), http.StatusInternalServerError)
            return
        }
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s", "service12", service18Resp)
    }

    if pathType == "2" {
        service17Resp, err := callService("http://service17:8080/api17", requestID, pathType, serviceIdx)
        if err != nil {
            http.Error(w, "Failed to call service4: "+err.Error(), http.StatusInternalServerError)
            return
        }
        ...
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s %s %s", "service12", service17Resp, service18Resp)
    }

    if pathType == "4" {
        w.Header().Set("RequestID", requestID)
        fmt.Fprintf(w, "%s", "service12")
    }
...
func main() {
    http.HandleFunc("/api12", apiHandler)
